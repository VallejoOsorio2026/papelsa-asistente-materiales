...
| PENDIENTE-020 | Orden de trabajo: 7 u 8 dígitos. En producción el comentario de `solicitudes_materiales.orden_trabajo` dice «Siete digitos exactos», pero la restricción (`NOT VALID`) y `enviar_solicitud_materiales()` exigen **8**, y el mensaje al usuario también dice 8. El `024` histórico dejaba 7; el `036` registra el paso a 8. M1-B reproduce producción tal cual y **no decide cuál es la regla de negocio**. Mientras siga abierto, el comentario de la columna contradice lo que la base valida. `tests/m1b_equivalencia.sh` falla si alguien corrige uno de los dos lados sin decisión. | Abierto | Confirmación del responsable |
| PENDIENTE-021 | Permisos amplios recuperados en M1-B, versionados sin endurecer. Las seis funciones de M1-B tienen `EXECUTE` para `PUBLIC`, `anon`, `authenticated` y `service_role`, y cinco son `SECURITY DEFINER` (todas salvo `es_material_baja`). Cuatro de las cinco comprueban `es_usuario_activo()`; `mi_historial` no: solo filtra por `auth.uid()`. `jerga_planta`, `busquedas_sin_resultado` y `solicitudes_materiales` conceden todos los privilegios de tabla a `anon`, `authenticated` y `service_role`: la única barrera es RLS. La política `solicitudes_mat_actualizacion` tiene `WITH CHECK true`: quien puede actualizar una fila puede dejarla con cualquier valor, incluida otra área. Revisar juntos grants, `SECURITY DEFINER` y RLS; cualquier `REVOKE` debe versionarse y probarse contra `js/`. | Abierto | Bloque de endurecimiento |
| PENDIENTE-022 | `sql/` no reconstruye la base de principio a fin. Detectado al reproducir en local durante M1-B: (1) `002_schema.sql` tiene sin cerrar la cadena del comentario de `inventario_materiales.ubicacion`, así que no se crean las tablas de operación (`solicitudes`, `solicitud_items`, `feedback`, `candidatos`, `decisiones`, `configuracion`, `sinonimos`, `banco_pruebas`...); (2) `025_correo.sql` tiene un fragmento suelto de `cuerpo_correo_solicitud()` detrás de su `$function$;`; (3) `027_reintento_correo.sql` necesita pg_cron; (4) `016_materiales_baja.sql` contiene las funciones de inactividad, y `017` y `026` figuran en el README pero no existen; (5) `028` y `029` no están en la tabla de orden. M1-B no corrigió nada de esto: la prueba de reproducción corrige el `002` solo en memoria para poder medir. | Abierto | Bloque de reproducibilidad |
